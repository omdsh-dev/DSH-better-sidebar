...
export interface SidebarPrefs {
   ...
   * Remote's `ctx.remote.session.openWorkspacePath`) open in the sidebar
   * editor instead of the Host OS's default application. On by default; the
   * editor tab's own enable switch gates it too (both must be on for the
   * takeover).
   */
