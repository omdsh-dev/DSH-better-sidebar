...
export const zhTW: Record<string, string> = {
  ...
  pluginIdeGitDesc: 'better-sidebar 的 Git 分頁:分支樹(ahead/behind 標記、worktree 佔用標記)、可按文字/分支/作者/日期/路徑篩選的提交圖譜、逐檔案逐行 diff、變更清單與提交框,還有合併、變基、揀選、重設、貯藏與標籤操作;破壞性操作會先確認,刪除與丟棄都能撤回。沒裝 better-sidebar 時會註冊到 DSH 原生右側欄,因此可以單獨使用',
