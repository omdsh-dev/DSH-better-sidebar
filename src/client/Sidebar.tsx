...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  const localeRevision = useSyncExternalStore(
    useMemo(
      () => (callback: () => void) => ctx.locale.subscribe(callback),
      [ctx],
    ),
  ...
  const summaryCwd =
    sessionId === undefined ? undefined : sessionList.byId[sessionId]?.cwd;
  ...
  const preset =
    scheme === "preset"
      ? getShellPreset(snapshot.prefs.titleBarPresetId)
      : undefined;
  ...
  useEffect(() => {
    ...
    if (presetCss !== "")
      tags.push(
        injectUserCss("data-dsh-preset-css", preset?.id ?? "", presetCss),
      );
    if (customCss !== "")
      tags.push(injectUserCss("data-dsh-custom-css", "custom", customCss));
    return () => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => {
        /* the explorer/git rows surface their own errors */
      });
    return () => {
      cancelled = true;
    };
  ...
  const { centerRectRef, centerMeasured, measureCenter, draggingRef } =
    useCenterColumn(bottomRef, state?.bottomOpen);
  ...
  const clampHeight = (height: number): number =>
    Math.min(
      ...
      Math.max(BOTTOM_MIN, window.innerHeight - CONVERSATION_MIN),
    );
  ...
  const abortDrag = (reset: () => void, event?: { clientY: number }): void => {
    ...
    if (pending !== null) {
      ...
      height = clampHeight(
        bottomDrag.current.startHeight +
          (bottomDrag.current.startY - event.clientY),
      );
    }
    if (height !== undefined) {
      ...
      dragCommitted.current = true;
      stopDragScheduling();
      const adopted = clampHeight(
        lastDragHeight.current ?? state?.bottomHeight ?? BOTTOM_MIN,
      );
      applyDrag(adopted);
      draggingRef.current = false;
      measureCenter();
  ...
  useEffect(() => {
    ...
    writeGeometry(
      snapshot.state?.bottomOpen === true ? height + keyboardInset : 0,
    );
  }, [
    ...
    () => ({
      closeTab: (paneId, tabId) => {
        ...
        const leaf =
          current === undefined
            ? undefined
            : leafWithTab(current.bottomSplits, tabId);
      ...
      moveTabToEdge: (
        ...
        store.reduce((s) =>
          moveTabToEdge(s, payload.paneId, payload.tabId, toPane, zone),
        );
      },
      moveTabBefore: (
        payload: TabDragPayload,
      ...
      pinTab: (tabId, scope) => {
        store.reduce((s) =>
          setTabPin(s, tabId, scope === null ? null : { scope, homeCwd: cwd }),
        );
      },
      ...
      addTabToConversation: (tabId) => {
        ...
        const tab = leaf?.tabs.find((candidate) => candidate.id === tabId);
        if (tab === undefined || tab.type !== "file" || tab.path === undefined)
          return;
  ...
  const referenceInChat = useCallback(
    (path: string, isDir: boolean): void => {
  ...
  const onNewTab = (optionId: string): void => {
    ...
    service.openTab(
      { type: optionId, title, target: "bottom" },
      { sessionId, cwd },
    );
  };
  ...
  const tabBadgeOf = (tab: SidebarTab): ReactNode => {
    ...
    if (isAgentTabId(tab.id)) {
      ...
      if (wait !== undefined)
        return <span className={css.tabBadge}>{"⏳"}</span>;
    }
    const descriptor = ctx.get("betterSidebar")?.getTab(tab.type);
    ...
    <div data-dsh-panel-host {...osFileDragShield}>
      ...
      */}
      <div
        ...
        className={clsx(
          css.bottomPanel,
          !state.bottomOpen && css.bottomPanelHidden,
        )}
        data-dsh-panel
      ...
      >
        <div
          className={clsx(
            css.bottomResize,
            draggingBottom && css.bottomResizeActive,
          )}
          onPointerDown={(event) => {
            event.preventDefault();
