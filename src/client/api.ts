...
/** One repository snapshot returned by git.status. */
...
export const api = {
  ...
  gitStatus: (scope: SessionScope, signal?: AbortSignal) =>
    call<GitRepositoryStatus[]>('git.status', scopePayload(scope, {}), signal),
  gitDiff: (scope: SessionScope, repository: string | undefined, path: string | undefined, staged: boolean, signal?: AbortSignal) =>
    call<{ diff: string }>('git.diff', scopePayload(scope, { ...(repository !== undefined ? { repository } : {}), ...(path !== undefined ? { path } : {}), staged }), signal),
  gitStage: (scope: SessionScope, repository: string, path?: string) =>
    call<{ ok: true }>('git.stage', scopePayload(scope, { repository, ...(path !== undefined ? { path } : {}) })),
  gitUnstage: (scope: SessionScope, repository: string, path?: string) =>
    call<{ ok: true }>('git.unstage', scopePayload(scope, { repository, ...(path !== undefined ? { path } : {}) })),
  gitCommit: (scope: SessionScope, repository: string, message: string) =>
    call<{ ok: true }>('git.commit', scopePayload(scope, { repository, message })),
  gitBranch: (scope: SessionScope, repository: string, signal?: AbortSignal) =>
    call<{ current: string; names: string[] }>('git.branch', scopePayload(scope, { repository }), signal),
  gitCheckout: (scope: SessionScope, repository: string, branch: string) =>
    call<{ ok: true }>('git.checkout', scopePayload(scope, { repository, branch })),
  /** Recent commit history, lazily pageable (skip/count; defaults 0/30). */
  gitLog: (scope: SessionScope, repository: string, count?: number, skip?: number, signal?: AbortSignal) =>
  ...
  gitCommitDiff: (scope: SessionScope, repository: string | undefined, hash: string, signal?: AbortSignal) =>
    call<{ diff: string }>('git.commit-diff', scopePayload(scope, { ...(repository !== undefined ? { repository } : {}), hash }), signal),
  /** Discard the worktree changes of one file (the index is untouched). */
  gitDiscard: (scope: SessionScope, repository: string, path: string) =>
    call<{ ok: true }>('git.discard', scopePayload(scope, { repository, path })),
  /** Revert one commit onto the current branch. */
  gitRevert: (scope: SessionScope, repository: string, hash: string) =>
    call<{ ok: true }>('git.revert', scopePayload(scope, { repository, hash })),
  /** Cherry-pick one commit onto the current branch. */
  gitCherryPick: (scope: SessionScope, repository: string, hash: string) =>
    call<{ ok: true }>('git.cherry-pick', scopePayload(scope, { repository, hash })),
