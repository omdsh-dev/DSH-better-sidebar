...
/** One recursive search result row (name or content match). */
...
export const api = {
  ...
  /** Recursively search a directory by file name and file content. */
  fsSearch: (scope: SessionScope, path: string, query: string, signal?: AbortSignal) =>
    call<SearchOutcome>('fs.search', scopePayload(scope, { path, query }), signal),
