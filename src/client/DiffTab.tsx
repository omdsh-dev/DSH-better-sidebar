...
export function DiffTab(props: { sessionId: string; cwd: string | undefined; diff: SidebarDiffRef }) {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        if (diff.kind === 'commit') {
          const result = await api.gitCommitDiff(scope, diff.repository, diff.hashFull)
          if (!cancelled) setData({ diff: result.diff })
          return
        }
        let result = await api.gitDiff(scope, diff.repository, diff.path, diff.staged)
        if (result.diff === '') {
          ...
          const other = await api.gitDiff(scope, diff.repository, diff.path, !diff.staged)
        ...
        if (diff.untracked === true && !diff.staged) {
          const path = diff.repository === undefined
            ? diff.path
            : `${diff.repository.replace(/[\\/]+$/, '')}/${diff.path}`
          const text = await api.fsRead(scope, path)
          if (!cancelled) {
