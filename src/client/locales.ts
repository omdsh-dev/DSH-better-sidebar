...
export const zh = {
  ...
  edit: '编辑',
  refresh: '刷新',
  explorerRootEditLabel: '编辑资源管理器根目录',
  explorerRootEditHint: '点击输入路径以切换根目录；目录以其本身为根，文件以其所在目录为根',
  settingsExplorerOutsideCwdTitle: '允许在会话目录外预览与下载',
  settingsExplorerOutsideCwdDesc: '在资源管理器中切换到会话 cwd 之外的目录后，允许其中的图片 / PDF / HTML 预览与文件下载（仅限在资源管理器中确认过的根目录；默认开启，关闭后恢复为仅限会话 cwd）',
  save: '保存',
  saved: '已保存',
  unsaved: '未保存',
  saveFailed: '保存失败',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  edit: 'Edit',
  refresh: 'Refresh',
  explorerRootEditLabel: 'Edit the explorer root',
  explorerRootEditHint: 'Click to enter a path and switch the root; a directory roots at itself, a file at its parent directory',
  settingsExplorerOutsideCwdTitle: 'Allow previews and downloads outside the session directory',
  settingsExplorerOutsideCwdDesc: 'After switching the explorer to a directory outside the session cwd, allow image / PDF / HTML previews and file downloads there (only under roots confirmed in the explorer; on by default — turning it off restores the strict session-cwd fence)',
  save: 'Save',
  saved: 'Saved',
  unsaved: 'Unsaved',
