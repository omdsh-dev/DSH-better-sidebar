...
export const zh = {
  ...
  settingsEditDiffTitle: '编辑类文件以 diff 视图打开',
  settingsEditDiffDesc: '开启后，点击编辑工具的文件链接时在侧边栏打开 git 工作区 diff 视图，而不是直接打开文件；无未提交改动时显示该文件最近提交的 diff，非 git 仓库自动回退为文件打开',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  settingsEditDiffDesc: 'When on, clicking an edit-tool file link opens the git worktree diff view in the sidebar instead of the file editor; when there is no unstaged change it shows the diff of the last commit that touched the file, and non-git workspaces fall back to the file',
