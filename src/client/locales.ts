...
export const zh = {
  ...
  settingsTitleBarDesc: '为覆盖在网页上的原生标题栏/壳顶栏预留空间。自动检测（默认）只使用标准的 Window Controls Overlay 几何；也可选用内置壳方案或自定义 CSS',
  ...
  settingsSchemePresetTitle: '壳兼容方案',
  settingsSchemePresetDesc: '为已知桌面壳应用内置适配（只收录 issue/PR 中出现过且 100+ star 的壳；手动启用，自动检测不会替你开启）',
  settingsPresetLabel: '桌面壳',
  settingsPresetDesc: '选择你正在使用的桌面壳；未列出时可用自定义方案或自定义 CSS 解决',
  settingsPresetDetected: '检测到当前环境可能是「{shell}」，建议启用该方案',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  settingsTitleBarDesc: 'Reserve space for native title bars / shell chrome drawn over the web content. Auto (default) only uses the standard Window Controls Overlay geometry; built-in shell presets and custom CSS are also available',
  ...
  settingsSchemePresetTitle: 'Shell preset',
  settingsSchemePresetDesc: 'Apply a built-in adaptation for a known desktop shell (only shells that appeared in this repo\'s issues/PRs with 100+ stars; opt-in — auto-detect never enables one for you)',
  settingsPresetLabel: 'Desktop shell',
  settingsPresetDesc: 'Pick the desktop shell you are running; shells not listed can be handled with the custom scheme or custom CSS',
  settingsPresetDetected: 'The current environment looks like «{shell}» — enabling this preset is recommended',
