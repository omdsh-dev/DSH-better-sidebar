...
export const zh = {
  ...
  chatMentionOpen: '在侧边栏中打开 {name}',
  settingsChatPathLinksTitle: '聊天中的路径/行号链接',
  settingsChatPathLinksDesc: '聊天代码片段中的文件路径（如 src/main.ts）与行号引用（如 src/main.ts:42 或 src/main.ts:42-56）变为可点击链接，点击后在侧边栏打开文件并跳转、高亮到对应行',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  chatMentionOpen: 'Open {name} in the sidebar',
  settingsChatPathLinksTitle: 'Path & line links in chat',
  settingsChatPathLinksDesc: 'File paths (e.g. src/main.ts) and line references (e.g. src/main.ts:42 or src/main.ts:42-56) in chat code snippets become clickable; clicking opens the file in the sidebar and jumps to, highlighting, the referenced lines',
