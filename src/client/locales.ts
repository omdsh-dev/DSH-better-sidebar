...
export const zh = {
  ...
  pluginHpptoolsMemoryDesc: '三层 Markdown 记忆系统（core-prompt / 会话小本本 / 长期记忆）：固化子代理每 5 轮自动沉淀对话、海马体手动整理；内置记忆控制台 tab（概览 / 文件 / 模型 / 运行 / 设置），记忆文件树 + Markdown 预览编辑',
  memTab: '记忆管理',
  memRefresh: '刷新',
  memNavOverview: '概览',
  memNavFiles: '文件',
  ...
  memUnitCount: '个',
  memRunning: '运行中',
  memOverviewFailed: '概览加载失败',
  memNoProject: '暂无项目记忆',
  memNoSession: '无活动会话',
  memNeverCleaned: '从未整理',
  memCardCurProject: '当前项目记忆',
  memCardAllProjects: '全部项目记忆',
  memCardGlobal: '全局记忆',
  memCardMaintenance: '最近记忆整理',
  memCardActiveRuns: '运行中的子代理',
  memCardExtractor: '固化模型',
  memCardCleaner: '海马体模型',
  memCardCurSub: '打开会话后可见',
  memCardAllSub: '所有项目汇总',
  memCardPersonalDir: 'personal/',
  memCardMaintenanceDir: 'maintenance/',
  memCardExtractorSub: 'extractor',
  memCardCleanerSub: 'cleaner',
  memMigDetected: '🔍 检测到 Pi agent 记忆（{{n}}），尚未迁移。迁移为复制操作，Pi 数据保持原样。',
  memMigrateNow: '开始迁移',
  memMigrated: '已从 Pi agent 迁移记忆（来源 {{from}}，{{n}} 项，{{at}}）。Pi 的数据保持原样。',
  memMigrateDone: '✅ 迁移完成',
  memSelectHint: '选择一个文件查看内容；✏️ 编辑可快捷修改。',
  memEmptyGroup: '（空）',
  memLoaded: '已加载（{{n}} 行）',
  memSaved: '✅ 已保存 {{path}}',
  memEdit: '编辑',
  ...
  memCancel: '取消',
  memEntryNav: '条目导航',
  memSubagentModels: '子代理模型',
  memExtractorModel: '固化子代理模型',
  memCleanerModel: '海马体模型',
  memSaveModels: '保存模型配置',
  memModelsLoaded: '已加载 {{n}} 个 provider',
  memModelsSaved: '✅ 已保存：固化={{a}}，海马体={{b}}',
  memModelSourceHint: '选项来自 DeepSeek Harness 已配置的模型（设置 → 模型）。选择 "(default)" 则使用当前会话的默认模型。',
  memRunsHint: '最近 {{total}} 次运行，{{active}} 个进行中（每 3 秒刷新）。点击行查看活动日志。',
  memRunClean: '运行记忆清理（海马体）',
  memCleanStarted: '✅ 海马体已启动（run {{id}}），稍等查看进度…',
  memRunConsolidate: '固化记忆',
  memConsolidateStarted: '✅ 固化子代理已启动（run {{id}}），稍等查看进度…',
  memColStarted: '开始时间',
  memColKind: '类型',
  memColStatus: '状态',
  memColDur: '持续',
  ...
  memBadgeDone: '完成',
  memBadgeFailed: '失败',
  memStorageTitle: '存储位置',
  memCurrentPath: '当前路径',
  memNewPath: '新路径',
  memSaveRoot: '保存（重启生效）',
  memCopyData: '同时把现有数据复制到新路径',
  memRootHint: '修改后需重启 DeepSeek Harness 生效。也可在组合行（cordis.patch.yml）里用 config.root 配置；UI 设置优先。',
  memRootPlaceholder: '例如 D:/my-memory 或 ~/memory',
  memRootSaved: '✅ 已保存新路径：{{root}}。重启后生效。',
  memRootSavedCopied: '✅ 已保存并复制 {{n}} 项数据。新路径：{{root}}。重启后生效。',
  memRootEmpty: '请输入新路径。',
  memMigrationTitle: 'Pi 记忆迁移',
  memMigrateNone: '未检测到 Pi agent 记忆。',
  memObsidianTitle: 'Obsidian / 文件管理',
  memObsidianHintText: '记忆是 Obsidian 兼容的 Markdown vault（[[双向链接]]、## 条目均为 Obsidian 语法）。可在 Obsidian 中把存储目录添加为 vault。',
  memOpenFolder: '打开存储文件夹',
}
...
export const en: Record<keyof typeof zh, string> = {
  ...
  pluginHpptoolsMemoryDesc: 'Layered Markdown memory system (core-prompt / session notebook / long-term memories): a consolidation subagent distills conversations every 5 rounds, a hippocampus subagent cleans on demand; powers the built-in memory console tab (Overview / Files / Models / Runs / Settings) with a memory file tree and Markdown preview/edit',
  memTab: 'Memory',
  memRefresh: 'Refresh',
  memNavOverview: 'Overview',
  memNavFiles: 'Files',
  memNavModels: 'Models',
  memNavRuns: 'Runs',
  memNavSettings: 'Settings',
  memLoading: 'Loading…',
  memUnitFiles: 'files',
  memUnitEntries: 'entries',
  memUnitProjects: 'projects',
  memUnitSkills: 'skills',
  memUnitCount: '',
  memRunning: 'running',
  memOverviewFailed: 'Overview failed to load',
  memNoProject: 'No project memory',
  memNoSession: 'No active session',
  memNeverCleaned: 'Never cleaned',
  memCardCurProject: 'Current Project',
  memCardAllProjects: 'All Projects',
  memCardGlobal: 'Global Memory',
  memCardMaintenance: 'Last Cleanup',
  memCardActiveRuns: 'Active Subagents',
  memCardExtractor: 'Extractor Model',
  memCardCleaner: 'Hippocampus Model',
  memCardCurSub: 'Visible after opening a session',
  memCardAllSub: 'All projects',
  memCardPersonalDir: 'personal/',
  memCardMaintenanceDir: 'maintenance/',
  memCardExtractorSub: 'extractor',
  memCardCleanerSub: 'cleaner',
  memMigDetected: '🔍 Pi agent memory detected ({{n}}), not migrated yet. Migration is a copy; Pi data stays intact.',
  memMigrateNow: 'Migrate now',
  memMigrated: 'Migrated from Pi agent (from {{from}}, {{n}} items, {{at}}). Pi data stays intact.',
  memMigrateDone: '✅ Migration complete',
  memSelectHint: 'Select a file to view; ✏️ Edit to modify quickly.',
  memEmptyGroup: '(empty)',
  memLoaded: 'Loaded ({{n}} lines)',
  memSaved: '✅ Saved {{path}}',
  memEdit: 'Edit',
  memPreview: 'Preview',
  memSave: 'Save',
  memCancel: 'Cancel',
  memEntryNav: 'Sections',
  memSubagentModels: 'Subagent Models',
  memExtractorModel: 'Extractor model',
  memCleanerModel: 'Hippocampus model',
  memSaveModels: 'Save model config',
  memModelsLoaded: 'Loaded {{n}} providers',
  memModelsSaved: '✅ Saved: extractor={{a}}, hippocampus={{b}}',
  memModelSourceHint: 'Options come from DeepSeek Harness configured models (Settings → Models). Choose "(default)" to use the session default.',
  memRunsHint: '{{total}} recent runs, {{active}} running (refreshed every 3s). Click a row for the activity log.',
  memRunClean: 'Run memory cleanup (hippocampus)',
  memCleanStarted: '✅ Hippocampus started (run {{id}}). Check back for progress…',
  memRunConsolidate: 'Consolidate memory',
  memConsolidateStarted: '✅ Consolidation subagent started (run {{id}}). Check back for progress…',
  memColStarted: 'Started',
  memColKind: 'Kind',
  memColStatus: 'Status',
  memColDur: 'Duration',
  memColReason: 'End reason',
  memNoRuns: 'No runs yet',
  memBadgeRunning: 'Running',
  memBadgeDone: 'Done',
  memBadgeFailed: 'Failed',
  memStorageTitle: 'Storage',
  memCurrentPath: 'Current path',
  memNewPath: 'New path',
  memSaveRoot: 'Save (restart required)',
  memCopyData: 'Copy existing data to the new path',
  memRootHint: 'Takes effect after restarting DeepSeek Harness. You can also set config.root in cordis.patch.yml; the UI setting wins.',
  memRootPlaceholder: 'e.g. D:/my-memory or ~/memory',
  memRootSaved: '✅ Saved new path: {{root}}. Restart to apply.',
  memRootSavedCopied: '✅ Saved and copied {{n}} items. New path: {{root}}. Restart to apply.',
  memRootEmpty: 'Please enter a new path.',
  memMigrationTitle: 'Pi Memory Migration',
  memMigrateNone: 'No Pi agent memory detected.',
  memObsidianTitle: 'Obsidian / Files',
  memObsidianHintText: 'Memory is an Obsidian-compatible Markdown vault ([[wikilinks]] and ## entries are Obsidian syntax). Add the storage dir as a vault in Obsidian.',
  memOpenFolder: 'Open storage folder',
}
