...
/** The ru dictionary (key-set-equal to zh, enforced by the type annotation). */
export const ru: Record<keyof typeof zh, string> = {
  ...
  terminal: 'Терминал',
  editor: 'Редактор',
  editorExplorer: 'Поведение при открытии файлов',
  editorExplorerDesc: 'Управляет тем, как открываются файлы',
  editorExplorerMerged: 'Объединённый',
  editorExplorerMergedDesc: 'Файлы переключаются на месте в одном окне; новые окна открываются с раскрытым деревом файлов',
  editorExplorerSplit: 'Раздельный',
  editorExplorerSplitDesc: 'Окна без пути — это отдельный проводник (только дерево файлов); каждый файл открывается в своём окне (дерево закреплено, по умолчанию свёрнуто)',
  editorTreeToggle: 'Панель дерева файлов',
  editorPathPlaceholder: 'Путь к файлу (относительно каталога сессии или абсолютный), Enter — открыть',
  editorSearchPlaceholder: 'Поиск файлов по имени…',
  editorSearchNoResults: 'Совпадений не найдено',
  editorSearchTruncated: 'Слишком много результатов — показана часть списка',
  editorEmptyHint: 'Выберите файл в дереве или введите путь выше, чтобы начать предпросмотр',
  openFileNewTab: 'Открыть в новой вкладке',
  openFileSide: 'Открыть сбоку',
  newTab: 'Новая вкладка',
  openExplorer: 'Проводник',
  brokenSymlink: 'Битая символическая ссылка',
  openGit: 'Панель Git',
  ...
  terminalError: 'Не удалось подключить терминал',
  ...
  terminalDepsHint: 'Выполните команду ниже в терминале или cmd на машине с DSH, чтобы исправить, затем нажмите «Повторить» (версия node-pty должна совпадать с версией ядра DSH):',
  terminalDepsProfile: ' (обнаружен профиль: {profile})',
  preview: 'Предпросмотр',
  edit: 'Изменить',
  mermaidError: 'Не удалось отрисовать диаграмму Mermaid',
  ...
  notRepo: 'Этот каталог не является репозиторием git',
  ...
  commit: 'Закоммитить',
  commitError: 'Не удалось выполнить коммит',
  branch: 'Ветка',
  checkoutError: 'Не удалось переключить ветку',
  history: 'История',
  changes: 'Изменения',
  staged: 'В индексе',
  unstaged: 'Вне индекса',
  cancel: 'Отмена',
  diffEmpty: 'Нет текстовых изменений',
  diffLoadError: 'Не удалось загрузить diff',
  diffBinary: 'Двоичный файл',
  diffAdded: 'Добавлено',
  diffDeleted: 'Удалено',
  diffRenamed: 'Переименовано',
  diffExpand: 'Показать ещё {count} строк',
  diffCollapse: 'Свернуть',
  discard: 'Отменить изменения',
  discardTitle: 'Отменить изменения',
  discardDesc: 'Изменения рабочей копии «{path}» будут отменены безвозвратно.',
  viewCommitDiff: 'Посмотреть diff коммита',
  copyShortHash: 'Скопировать короткий хеш',
  copyFullHash: 'Скопировать полный хеш',
  copySubject: 'Скопировать сообщение коммита',
  revertCommit: 'Отменить этот коммит',
  revertTitle: 'Отменить этот коммит',
  revertDesc: 'В текущей ветке будет создан новый коммит, отменяющий «{subject}».',
  cherryPickCommit: 'Перенести этот коммит',
  cherryPickTitle: 'Перенести этот коммит',
  cherryPickDesc: 'Изменения из «{subject}» будут применены к текущей ветке.',
  ...
  copyRelative: 'Скопировать относительный путь',
  copyAbsolute: 'Скопировать абсолютный путь',
  download: 'Скачать',
  ...
  settingsPopupDesc: 'Настройки для «{feature}»',
  settingsDone: 'Готово',
  settingsOpenTitle: 'Открывать по умолчанию для новых диалогов',
  settingsOpenDesc: 'Автоматически разворачивать боковую карточку для новых диалогов; существующие диалоги сохраняют свою раскладку',
  settingsWidthTitle: 'Ширина по умолчанию',
  settingsWidthDesc: 'Доля ширины окна, занимаемая боковой карточкой по умолчанию для новых диалогов (20–60)',
  settingsWidthSuffix: '%',
  settingsOpenPathTitle: 'Открывать файлы из чата в боковой панели',
  settingsOpenPathDesc: 'Открывать ссылки на файлы в чате (строки инструментов, полученные файлы, упоминания) в редакторе боковой панели вместо системного приложения по умолчанию',
  settingsTitleBarTitle: 'Режим совместимости расположения',
  settingsTitleBarDesc: 'Резервировать место под нативную строку заголовка Windows справа сверху: кнопки и содержимое боковой панели смещаются вниз, чтобы не перекрываться заголовком',
  settingsTitleBarStripTitle: 'Величина смещения',
  settingsTitleBarStripDesc: 'Высота полосы заголовка: на сколько пикселей смещаются вниз кнопки и содержимое боковой панели (0–120, по умолчанию 40)',
  settingsSchemeAutoTitle: 'Автоопределение',
  settingsSchemeAutoDesc: 'Консервативный режим: учитывается только стандартный API Window Controls Overlay (реальная высота наложения заголовка); в обычном веб-окружении изменения не применяются',
  settingsSchemeWebTitle: 'Официальный веб DSH',
  settingsSchemeWebDesc: 'Явно указать, что используется официальный веб-интерфейс: адаптация не выполняется вовсе (даже стандартная геометрия WCO)',
  settingsSchemeCustomTitle: 'Своя схема',
  settingsSchemeCustomDesc: 'Полный контроль: внедрение собственного CSS (может переопределять встроенные стили) и указание величины смещения строки заголовка',
  settingsSchemeDetectedSuffix: 'обнаружено',
  settingsCustomCssTitle: 'Свой CSS',
  settingsCustomCssDesc: 'Стили, добавляемые в конец страницы (при равной специфичности побеждает последний; чтобы переопределить встроенные переменные, заданные из JS, используйте !important)',
  settingsCustomCssPlaceholder: '/* напр.: зарезервировать 36px для оболочки со своей строкой заголовка */\nhtml[data-dsh-title-bar-height="36"] {\n  --dsh-title-bar-strip: 36px !important;\n}',
  settingsSaveFailed: 'Не удалось сохранить',
  settingsConflict: 'Настройки были изменены в другом окне — повторите попытку',
  binaryNoPreview: 'Предпросмотр для этого типа файлов не поддерживается',
  downloadToView: 'Скачать для просмотра',
  settingsSubagentTitle: 'Автоматически открывать страницу задач при появлении субагента',
  settingsSubagentDesc: 'При появлении нового субагента в текущем диалоге автоматически разворачивать боковую панель и открывать страницу задач; при отключении нужно открывать вручную',
  settingsJobsTitle: 'Автоматически открывать страницу фоновых задач при новой задаче',
  settingsJobsDesc: 'При появлении новой фоновой задачи в текущем диалоге автоматически разворачивать боковую панель и открывать страницу фоновых задач (срабатывает на каждую новую задачу); при отключении нужно открывать вручную',
  settingsToolsTitle: 'Предоставить модели инструменты терминала',
  settingsToolsDesc: 'При включении модель может создавать и управлять терминалами боковой панели через 8 инструментов terminal_* (по умолчанию отключено)',
  settingsBottomTerminalTitle: 'Автоматически открывать терминал при первом раскрытии нижней панели',
  settingsBottomTerminalDesc: 'При первом раскрытии нижней панели в сессии пытаться автоматически открыть новую вкладку терминала (лимит терминалов по-прежнему действует; по умолчанию включено)',
  settingsFontFamilyTitle: 'Шрифт терминала',
  settingsFontFamilyDesc: 'Собственное семейство шрифтов терминала (стек CSS font-family, например "JetBrains Mono", monospace; оставьте пустым, чтобы использовать моноширинный шрифт темы)',
  settingsFontFamilyPlaceholder: '"JetBrains Mono", monospace',
  settingsFontSizeTitle: 'Размер шрифта терминала',
  settingsShellTitle: 'Путь к оболочке',
  settingsShellDesc: 'Оболочка, запускаемая для терминалов интерфейса и модели (абсолютный путь или имя исполняемого файла). Пусто — используется прежний порядок: config.shell из yaml → $SHELL / login shell / powershell.exe в Windows. Применяется к терминалам, открытым после изменения',
  settingsShellPlaceholder: 'например /bin/zsh (пусто = автоопределение)',
  settingsShellArgsTitle: 'Аргументы оболочки',
  settingsShellArgsDesc: 'Явные аргументы запуска оболочки через пробел; если заданы, полностью заменяют аргументы по умолчанию (соответствует контракту shellArgs из yaml)',
  settingsShellArgsPlaceholder: 'например -l (пусто = аргументы по умолчанию)',
  settingsFontSizeDesc: 'Размер шрифта терминала в px (9–32, по умолчанию 13)',
  settingsFontSizeSuffix: 'px',
  settingsTabsTitle: 'Содержимое боковой панели',
  settingsViewersTitle: 'Просмотр файлов',
  settingsGeneralTitle: 'Общие',
  settingsPopup: 'Настройки функции',
  settingsViewerCatchAll: 'Резервный: любой файл',
  viewerImage: 'Изображение',
  viewerPdf: 'PDF',
  viewerMarkdown: 'Markdown',
  viewerCode: 'Код',
  viewerBinary: 'Скачивание двоичного файла',
  viewerHtml: 'HTML',
  browser: 'Браузер',
  ...
  browserStart: 'Введите адрес, чтобы начать просмотр (режим песочницы)',
  browserBlockedScheme: 'Заблокировано: поддерживаются только ссылки http/https',
  browserBlockedLoopback: 'Заблокировано: доступ к локальным и внутренним адресам из браузера запрещён',
  browserInvalid: 'Некорректный адрес',
  browserNoSandboxWarning: 'Песочница отключена: текущая страница выполняется в том же источнике, что и интерфейс, с полными правами сессии (можно восстановить в настройках)',
  htmlNoSandboxWarning: 'Песочница отключена: этот HTML выполняется в том же источнике, что и интерфейс, и может читать файлы сессии и внутренние API (можно восстановить в настройках)',
  sandboxStatusOn: 'Режим песочницы: включён · страницы не имеют доступа к данным интерфейса и локальным файлам; авторизация и сторонние cookie могут не работать',
  sandboxUnlock: 'Временно отключить (небезопасно)',
  sandboxRestore: 'Восстановить песочницу',
  settingsHtmlDefaultUnsafeTitle: 'Открывать предпросмотр HTML без песочницы по умолчанию (небезопасно)',
  settingsHtmlDefaultUnsafeDesc: 'При включении каждый новый предпросмотр HTML открывается без песочницы (в том же источнике, что и интерфейс, с доступом к файлам сессии и внутренним API); в строке состояния можно временно восстановить песочницу',
  settingsHtmlSandboxTitle: 'Отключить песочницу предпросмотра HTML (небезопасно)',
  settingsHtmlSandboxDesc: 'При отключении песочницы предпросматриваемый HTML выполняется в том же источнике, что и интерфейс: может читать файлы сессии, локальное хранилище и вызывать внутренние API. Включайте только для полностью доверенных файлов',
  settingsBrowserSandboxTitle: 'Отключить песочницу браузера (небезопасно)',
  settingsBrowserSandboxDesc: 'При отключении песочницы любой открытый сайт выполняется в том же источнике, что и интерфейс: может читать данные сессии и действовать от имени вашей учётной записи. Включайте только для полностью доверенных сайтов',
  settingsBrowserLinksTitle: 'Открывать внешние ссылки из чата в боковой панели',
  settingsBrowserLinksDesc: 'При включении клик по внешней ссылке в чате или интерфейсе открывает боковую панель вместо нового окна; HTTP и HTTPS настраиваются отдельно переключателями ниже; Ctrl/Cmd+клик всегда открывает в обход',
  settingsBrowserHttpTitle: 'Открывать HTTP-страницы в боковой панели',
  settingsBrowserHttpDesc: 'При включении клик по внешней HTTP-ссылке в чате или интерфейсе открывает боковую панель (страницы плагинов с urlTarget имеют приоритет); Ctrl/Cmd+клик всегда открывает в обход',
  settingsBrowserHttpsTitle: 'Открывать HTTPS-страницы в боковой панели',
  settingsBrowserHttpsDesc: 'При включении клик по внешней HTTPS-ссылке в чате или интерфейсе открывает боковую панель. По умолчанию отключено: большинство HTTPS-сайтов запрещают встраивание, поэтому системный браузер работает надёжнее',
  browserOpenExternal: 'Открыть в браузере',
  browserEmbedBlocked: '{host} отклонил запрос на встраивание',
  browserEmbedBlockedDesc: 'Этот сайт запрещает отображение внутри других страниц (X-Frame-Options / frame-ancestors), поэтому загрузить его в боковой панели нельзя. Откройте его напрямую в браузере.',
  browserEmbedAnyway: 'Всё равно загрузить',
  subagent: 'Задачи',
  openSubagent: 'Задачи',
  subagentMainAgent: 'Главный агент',
  subagentEmpty: 'Нет субагентов',
  subagentEmptyDesc: 'Здесь будут отображаться субагенты, порождённые главным агентом',
  subagentRunning: 'Выполняется',
  subagentInactive: 'Неактивен',
  ...
  subagentDiagCorrupt: 'Повреждено',
  subagentDiagUnsupported: 'Не поддерживается',
  subagentDiagUnavailable: 'Недоступно',
  subagentThinking: 'Думает…',
  sideChat: 'Боковой чат',
  sideChatNew: 'Новая ветка',
  sideChatUntitled: 'Новая ветка',
  sideChatEmpty: 'Нет боковых диалогов',
  sideChatEmptyDesc: 'Каждый боковой диалог — отдельная вкладка на панели вкладок: он наследует контекст текущей сессии и никогда не попадает в основной диалог',
  sideChatCreating: 'Создание бокового диалога…',
  sideChatRetry: 'Повторить',
  sideChatThreads: 'Переключить ветку / создать',
  sideChatSave: 'Сохранить как новую сессию',
  sideChatSaveTitle: 'Превратить эту ветку в сессию верхнего уровня в основном списке сессий',
  sideChatSaved: 'Сохранено как новая сессия',
  sideChatNoTurn: 'Сохранение доступно после первого завершённого хода',
  sideChatPendingDrop: 'Последний оставшийся без ответа вопрос не войдёт в сохранённую сессию',
  sideChatFirstPlaceholder: 'Задайте первый вопрос — контекст унаследован…',
  sideChatComposerPlaceholder: 'Задать уточняющий вопрос…',
  sideChatThinking: 'Углубляюсь…',
  ...
  sideChatCloseTitle: 'Освободить агента этой ветки (история сохраняется)',
  ...
  jobNotReadYet: 'Ожидание чтения вывода моделью (после вызова job_output вывод появится здесь)',
  jobOutputTruncated: 'Вывод слишком длинный, показан частично',
  jobOutputError: 'Не удалось прочитать вывод',
  jobKill: 'Прервать',
  jobKillConfirm: 'Нажмите ещё раз для подтверждения',
  jobKillError: 'Не удалось прервать',
  ...
  addPluginsViewerCardDesc: 'Зарегистрировать предпросмотр для типа файлов',
  addPluginsTabDesc: 'Страницы боковой панели (вкладки) можно расширять плагинами. Плагины регистрируются через сервис ctx.betterSidebar; нажмите «Установить», чтобы скопировать команду установки, и выполните её в терминале на машине с DSH.',
  addPluginsViewerDesc: 'Средства предпросмотра файлов можно расширять плагинами. Плагины регистрируются через сервис ctx.betterSidebar; нажмите «Установить», чтобы скопировать команду установки, и выполните её в терминале на машине с DSH.',
  addPluginsBrowseMore: 'Посмотреть больше плагинов на GitHub (тема: dsh-better-sidebar)',
  addPluginsSearch: 'Поиск по названию или описанию плагина…',
  addPluginsNoMatch: 'Совпадающих плагинов не найдено',
  addPluginsRecommended: 'Рекомендуемые плагины',
  addPluginsEmpty: 'Плагины пока не добавлены — опубликуйте свой под темой GitHub',
  openPlugin: 'Открыть',
  copyInstall: 'Скопировать команду установки',
  pluginOfficeDesc: 'Предпросмотр офисных документов (.docx / .xlsx / .pptx) для редактора better-sidebar, вынесен из основного пакета для установки по требованию',
  pluginGitRemotesDesc: 'Вкладка Git-удалённых репозиториев для better-sidebar: ветки/upstream/ahead-behind, fetch (с опциональным prune), ff-only pull, push только после подтверждения. Не заменяет встроенную вкладку Git (индекс/коммит) и не предоставляет force-push или автоматический push моделью',
  pluginSentinelDesc: 'Система пробуждения агента по условиям: датчики файлов/процессов/портов/HTTP/команд/webhook пробуждают спящие сессии при срабатывании условий; регистрирует вкладку «Sentinel» с общесерверной таблицей наблюдения',
  pluginSidebarQaDesc: 'Выделение и вопрос на основе better-sidebar: выделите текст диалога → задайте вопрос в панели справа → отдельная сессия уточняющих вопросов (❓уточнение·тема) в том же рабочем пространстве; быстрая модель без рассуждений сжимает контекст основного диалога и добавляет его вместе с цитатой, не прерывая основной диалог; уточнения можно вкладывать друг в друга, продолжать и архивировать',
  pluginVideoPreviewDesc: 'Встроенный предпросмотр видеофайлов (.mp4/.webm/.mov/.mkv/.avi и др.) в редакторе better-sidebar с собственным серверным маршрутом /video с поддержкой HTTP Range (206): можно перематывать, ограничение mediaLimit 20 МБ не действует',
}
...
export function t(key: CopyKey, params?: Record<string, string | number>): string {
  ...
  const dict = locale.startsWith('zh') ? zh : locale.startsWith('ru') ? ru : en
  let text = dict[key]
  if (params !== undefined) {
