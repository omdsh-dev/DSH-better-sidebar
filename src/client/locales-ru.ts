...
export const ru: Record<string, string> = {
  ...
  pluginIdeGitDesc: 'Вкладка Git для better-sidebar: дерево веток (значки ahead/behind, метка worktree), граф коммитов с фильтрами по тексту/ветке/автору/дате/пути, построчный diff по файлам, список изменений и поле коммита, а также merge, rebase, cherry-pick, reset, stash и теги; разрушительные действия требуют подтверждения, удаление и откат изменений можно отменить. Без better-sidebar регистрируется в штатной правой панели DSH и работает самостоятельно',
  ...
  pluginIdeGitName: 'dsh-ide-git Окно Git',
