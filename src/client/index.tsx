 ...
 *  locale service backs the sidebar's copy — see locales.ts). `remote` +
 *  `remote.session` (0.1.2-alpha.x) is the Remote gateway whose
 *  `session.openWorkspacePath` the open-path interception wraps — the
 *  `remote.session` sub-path must be declared too (Cordis guards each path
 *  segment; ui-deliverables declares the same pair). `modules` (rc.8+) is the
 *  client module system the chunk loader resolves its externals through;
 *  `connection` (0.1.2-alpha.2+) is the Remote transport's recovery lifecycle
 *  the side chat's disconnect banner reads — Cordis guards service access
 *  without inject. */
