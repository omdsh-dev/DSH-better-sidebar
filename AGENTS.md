...
- **`src/client/FileTree.tsx`** / **`src/client/TreePanel.tsx`** / **`src/fs-operations.ts`** / **`src/fs-search.ts`**：受控文件树组件（右键菜单含新 Tab/侧边打开与区别文件、目录、符号链接的删除入口）/ 带二次确认的树面板 / host 侧受工作区边界约束的上传与递归删除 / 递归文件名搜索（`fs.search` 路由，预算兜底 + 跳过 `.git` / `node_modules` / 构建缓存 / symlink 目录；测试 `tests/tree-panel-delete.spec.tsx`、`tests/fs-operations.spec.ts`、`tests/fs-search.spec.ts`、`tests/editor-host.spec.tsx`）
