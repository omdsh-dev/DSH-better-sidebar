...
12. **`conversation.chat.turnTail` 的 kind 随宿主而变（0.1.6-alpha.2 起 chain → list）**：0.1.5-rc.2 及更早是 chain（选择器选举，`select` 返回 null 让位给官方交付卡片，插件用 `priority: -1` 接管）；0.1.6-alpha.2 起是 list（每个条目都渲染，`options.id` 必填，缺 id 时 `ctx.slots.register` 直接抛 `list slot ... requires options.id`，issue #705）。list 条目拿到的是 turn owner props，**没有 `matched` prop**（那是 chain 专属），所以 `select` 的判据要整段搬进条目组件、无内容时返回 `null`（宿主 `ui-deliverables` 的 `DeliverablesTail` 即规范写法）。`src/client/intercept.tsx` 因此在 `slots.inject` 回调里读 `ctx.slots.spec?.('conversation.chat.turnTail')?.kind` 分流两种注册形态（回调内声明已提交，读得到 spec；没有 `spec` 方法的宿主早于 list 契约，按 chain 处理），两条路径由 `tests/turn-tail-intercept.spec.ts` 守护。list 下不存在接管，本插件的产出文件行与官方交付卡片并列渲染（issue #390 的「顶掉第三方 turnTail 插件」在 list 契约下自然消失）。
