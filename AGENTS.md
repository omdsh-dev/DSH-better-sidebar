...
5b. **聊天文件打开的拦截点是 `ctx.remote.session.openWorkspacePath`，不是 `ctx.workspaces.openPath`**：ui-chat 的 `apply.ts` 是 `openFile` 的唯一生产调用方，直接 `ctx.remote.session.openWorkspacePath({ path: resolveWorkspacePath(cwd, path) })`；`ctx.workspaces`（WorkspaceController）只有 create/rename/delete/archive，从未定义过 `openPath`——包装一个不会被调用的方法等于接管永不发生（#500）。`wrapOpenWorkspacePath`（`src/client/openpath-intercept.ts`）包装 Remote session 方法，被接管调用以 `{ ok: true, value: { opened: true } }` 解析（调用方只查 `result.ok`）；client half 的 `inject` 用 `'remote'`（`dsh-api-remotes/client` 的类型合并提供 `Context['remote']`，ui-tool 同款 inject）。
