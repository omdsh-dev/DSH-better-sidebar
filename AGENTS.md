...
| `sidechat` | 35 | 是 | 否 | 侧边对话（Codex 风格）：线程 = 插件自建子会话（自定义种子继承父会话完整上下文，进行中回合以 `interrupted` 冻结诚实闭合），`origin:'subagent'` 隐藏于主列表、无 descriptor 零目录噪音；生命周期走自有 `/sidebar/api/sidechat.*` 路由（`ctx.agents.create/resume` + `agent.followup/cancel`，绕开 agent-lookup 所有权 fence）；「保存为新会话」= `session.fork` 提升顶层会话。见 [设计文档](docs/plans/2026-08-20-sidechat-tab-design.md) |
