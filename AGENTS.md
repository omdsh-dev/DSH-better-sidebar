...
11. **自定义种子必须带 fork 标记对（`meta.isSeeded: true` + `inheritedEventCount`），且 create 后必须 `inbox.clear()`——标记对必要但不充分**：dsh-session 契约「只给 seed 不标 isSeeded，种子算重放历史而非继承前缀」——缺标记时 `Session.ownEvents()` 含整个种子。**但 0.1.5-rc.2 的运行时 inbox 不走 `ownEvents()`**：`ReactLoopInbox.current()` → `ctx.sessionProjections.stateOf()` → `SessionProjectionRegistry.cellFor` 折的是 `snapshotEvents()` **全量日志（含继承种子前缀）**，且标准 inbox 投影的 `init` 忽略 `inheritedEventCount`、注册表 `session/created` 急切初始化跳过一切种子会话（seq ≠ 0）——种子里的 `agent/inbox/spliced` 会原样重放进子会话 live inbox（长对话 mid-turn 切割时即「排队信息漏入」复发：首问 claim 先排空 next-step，幽灵消息先于 boundary+问题发给模型）。因此 sidechat 宿主 `agents.create` 成功后、任何 inject/followup 之前必须 `handle.agent.inbox.clear()`（公开 API；durable 补偿 splice 是 own 事件，冷恢复折存储日志同样归零，转录不渲染 splice；无竞态——driver 只在 wake 后 claim，清空前无人 send 过）。已知限制：修复前创建的旧线程冷恢复仍可能漏（不在 resume 清，避免误杀合法排队输入）。回归由 `tests/sidechat-seed-validation.spec.ts`（真实 `Session.create` + **真实 `SessionProjectionRegistry`** 折全量日志：一测钉住上游洞、一测证明 clear 补偿归零）与 `tests/sidechat-routes.spec.ts`（clear 在 inject/followup 之前的时序断言）守护。宿主 `session.fork`（api-session-controller）的调用即规范形态；`AgentRegistry.create` 接缝补齐标记即为规范用法；`Session.create` 的 header `version` 必须用 `SESSION_FORMAT_VERSION`（0.1.5 是 `3` 字面量类型），勿钉 `0`。设计全文见 [docs/plans/2026-09-13-sidechat-inbox-projection-leak.md](docs/plans/2026-09-13-sidechat-inbox-projection-leak.md)。
