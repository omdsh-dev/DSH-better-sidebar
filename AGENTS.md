...
- **`src/client/FileTree.tsx`** / **`src/client/TreePanel.tsx`** / **`src/fs-search.ts`**：受控文件树组件（纯树体，文件行右键菜单含「在新 Tab 中打开」「在侧边打开」「以默认应用打开」，仅宿主编排提供回调时渲染）/ 树面板（搜索框 + 刷新 + FileTree，文件窗口的内嵌 dock 使用）与 host 侧递归文件名搜索（`fs.search` 路由，预算兜底 + 跳过 `.git`/symlink 目录；测试 `tests/fs-search.spec.ts`、组件测试 `tests/editor-host.spec.tsx`）
- **`src/client/TabBar.tsx`**：tab 栏右键菜单（关闭此/右侧/左侧/其他标签；文件标签的复制绝对/相对路径与「以默认应用打开」经 `getTabPath`/`onOpenFileSystem` 由 Sidebar 壳提供；测试 `tests/tab-bar-context-menu.spec.tsx`、`tests/tab-bar-wheel.spec.tsx`）；「以默认应用打开」经 `src/client/openpath-intercept.ts` 的 `openPathWithSystem` 走拦截前的原始 openPath，绕过聊天区文件打开的侧边栏接管
