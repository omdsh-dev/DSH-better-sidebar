...
// Every run builds its own scratch tree: the CI runner and a real machine
// have no shared fixed path (the original hard-coded C:\Users\y\wintest).
...
// The default-shape runs are DEMO cases (un-pinned output leaks '\' by
// design — that is exactly the review concern on #303); only the pinned
// (--path-separator /) and Chinese-name runs assert. `--exclude .git` keeps
// a worktree's `.git` pointer file out of the listing the engine emits.
