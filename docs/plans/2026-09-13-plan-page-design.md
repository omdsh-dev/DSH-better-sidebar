...
> ⚠️ 本节是**重构前**的设计。数据面已被推翻（折叠收归宿主、镜像与游标撤除），见文末「实施偏差」——下表只有 #5 已就地标注。
...
| 5 | 宿主**按工具名预筛**，折叠放客户端 ~~（重构后改为宿主侧折叠，见文末「实施偏差」）~~ | 复用 `changes.ops` 不行：那条路由只回最近 4000 条 tool 事件，长会话会把最早的几版计划挤出窗口，而「逐版留档」不允许丢。预筛后套 `afterSeq`，增量只传新行 |
...
> ⚠️ 同上：折叠与数据面两行已按重构后的事实改写，其余为重构前的清单。

| 子系统 | 文件 | 变更 |
|---|---|---|
| 宿主：路由与推送 | `src/plans-routes.ts`（新） | `buildPlansApi(ctx, limit)` 的 `events`（live → 冷读 → 空窗口）；`createPlanPushes(ctx)` 的 per-session 订阅者集合（无队列无重放、`ctx.on` 缺失时降级）；`PlanEventLike` 让匹配器同时吃宿主事件与插件行类型 |
...
| 宿主：折叠 | `src/plan-events.ts`（`derivePlans`） | 按 callId 配对、跳过全部畸形行（含 `/^#\s+\S/` 不过的幻影计划）、时间正序；宿主在 `plans.events` 里跑一次并下发折好的列表 |
| 客户端：页面 | `src/client/plans/PlanView.tsx` + `plan.module.css`（新） | `usePolling(visible, …, 5s, self-scheduling, immediate)` 每次整体替换列表；`<select>` 版本选择器 + 状态/时间标签 + 复制全文；`MarkdownDocument` 渲染，`media` memo 在原语上 |
...
| 客户端：自动打开 | `src/client/sidebar/use-host-feeds.ts` | `activateTasksPage` 泛化为 `activatePage(ctx, sessionId, type, options)`（三个既有调用点改传 `'subagent'`，行为逐字保持；标题改由 `service.openTab` 从 descriptor 兜底）；新增第三个 WS 订阅 effect，三道闸门后 `activatePage(…, 'plan', { background: true })`，并在闸门**之前**派发 window 刷新事件 |
| 客户端：数据面 | `src/client/api.ts` | `plansEvents(scope, signal?)` → 折好的 `PlanList`（无 `afterSeq`，页面不再持游标） |
...
- 新增 `tests/plans-ops.spec.ts`（纯 fold，含幻影计划与折叠幂等）、`tests/plans-routes.spec.ts`（假 ctx + 事件工厂，含判据、成对截断、seq-0 游标）、`tests/plan-view.spec.tsx`（mock `plans.events` 的页面行为）、`tests/session-store-window.spec.ts`（共享的会话窗口助手）。
...
## 实施偏差：折叠收归宿主、镜像与游标撤除（2026-09-13，同分支重构）

上线前的一次重构，把本文最初的数据面推翻，功能一字未改。

**为什么改**：计划页原本不是「显示服务器给的结果」，而是自己维持一份事件窗口——攒增量、去重竞态、剪 2000 行窗口、再折一遍；宿主那边则按工具名预筛同一批行、还要把镜像拼进日志。同一件事写了两遍，两边的判据就有漂移的机会，代码里最重的部分全在这个数据面上。

**改法**：

- `extractPlans` 从 `src/client/plans/ops.ts` 整体搬进 `src/plan-events.ts`（本就是两端共享的契约模块），由宿主在 `plans.events` 里跑一次并**直接下发折好的列表**；`src/client/plans/ops.ts` 删除，页面只持有服务器给的列表。协议随之从 `{ events, lastSeq }`（`afterSeq` 增量）变成**整份列表**——响应本身就代表当前状态，页面不再需要游标。
- 页面侧的**事件窗口、`seqRef` 游标、竞态去重、2000 行裁剪全部删除**；空闲轮询不做摘要比对，直接整体替换列表（正文未变 ⇒ markdown 分析与消毒都由按正文串的 memo 短路，代价只是 React 的一次浅比较）。
- **镜像与合并通道撤除**：`PlanRowMirror`、镜像环形缓冲（400 行 / 64 会话）、`sessionEventWindow` 的 `extraRows` 参数与两指针合并全部删除。它原本兜的是「宿主重启后 store 日志冻结在 rehydration 边界」，但只有计划路由用它，且代价（实现 + 测试约 190 行）远大于收益。

**已知能力回退（唯一一处，明确接受）**：宿主重启后若日志恰好冻结，该窗口内提交的计划**暂时不出现在页面上，刷新即自愈**（走持久化读取）。这是本次重构唯一的降级项，选择它的理由是：代价是「晚一点看到」，而保留它的代价是一整套只有本功能使用的镜像机制。若真机确认这是实际问题，回补路径很小（镜像只存计划行，读窗前对 `seq` 做一次并集）。

**保留的一条硬规则**（来自这次重构的对抗性复核，有测试守护）：**空响应绝不覆盖已显示的列表**。读不到日志（宿主重启、瞬时失败）与「这个会话没有计划」是两回事；旧实现只追加不替换天然安全，改成整体替换后必须显式守住，否则一次读取失败就会把用户正在看的多版历史洗成空页。

**没有采纳的方向**（做过一轮多视角方案竞争与对抗验证）：宿主侧按会话缓存折叠结果 + 版本令牌（`{ entries, revision }`）+ 正文按摘要按需下发 + 提交去抖探测 + 结算也推送。它能让稳态轮询零流量、状态从「≤5 秒」变亚秒级，但净增约 65 行、两个新文件，并引入缓存失效、去抖窗口、跨包探测协议三类新失效模式；而宿主每轮本来就要整份解析日志（`abortedPlanCallIdsOf` 已是全量扫描），令牌省的只是几十 KB 的本地流量。**结论：不值得**。
