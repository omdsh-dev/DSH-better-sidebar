...
| `plan` | 25 | 是 | 否 | 计划页：本会话内模型经宿主 plan mode 的 `exit_plan_mode` 工具提交的每份计划全文，逐版留档、页内下拉切换，每版带评审状态（待评审 / 已批准 / 未采纳）。数据经插件自有宿主路由 `plans.events` 供给，**宿主折叠后直接下发计划列表**（无游标、无增量：一次响应就是当前全部版本）；计划行按工具名预筛，live 日志优先、冷会话回放持久化记录；模型提交的瞬间由 `/sidebar/ws/plans` 推送 `{sessionId, seq}` 触发静默切页（不抢焦点、窄屏停放）。**零磁盘写入**：计划只存在于会话事件日志与页面上。[设计文档](plans/2026-09-13-plan-page-design.md) |
