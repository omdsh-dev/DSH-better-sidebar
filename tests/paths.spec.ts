...
describe('path helpers', () => {
  ...
  it('htmlUrl always marks UNC paths (platform-neutral marker)', () => {
    ...
    // A scope cwd additionally rides as the leading '$c' hint segment (it
    ...
    expect(htmlUrl({ sessionId: 's', cwd: '/home/me' }, '//server/share/a.html'))
      .toBe(`/sidebar/html/s/$c${encodeURIComponent('/home/me')}//server/share/a.html`)
    expect(htmlUrl({ sessionId: 's', cwd: '/home/me' }, '/home/me/index.html'))
      .toBe(`/sidebar/html/s/$c${encodeURIComponent('/home/me')}/home/me/index.html`)
  })

  it('htmlUrl marks a workspace-relative path with $r so the host joins it onto the cwd', () => {
    // DSH file addresses spell a workspace file relatively, so this is the
    // common shape for a file opened from chat.
    expect(htmlUrl({ sessionId: 's', cwd: 'C:\\DshChat' }, 'proj/index.html'))
      .toBe(`/sidebar/html/s/$c${encodeURIComponent('C:\\DshChat')}/$r/proj/index.html`)
  })
})
