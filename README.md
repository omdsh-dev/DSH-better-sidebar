...
- **🔗 路径/行号直达**：聊天中模型输出的文件路径与 `path:line`（如 `src/main.ts:42`）自动可点击，点击即在侧边栏打开并滚动高亮到对应行；**仅真实存在的文件会生成链接**（工作区目录树扫描缓存），不存在的路径保持纯文本
...
- 🔗 **聊天区路径/行号超链接**：聊天代码片段中的文件路径（`src/main.ts`）与行号引用（`src/main.ts:42`、`src/main.ts:42-56`）自动变为可点击链接，点击后在侧边栏编辑器打开文件，并**滚动 + 高亮**到对应行（Markdown/HTML 自动切到编辑模式预览）
- 🗂️ **只链接真实存在的文件**：会话打开工作区时自动做一次**有边界的目录树扫描**（跳过 node_modules/.git/dist 等重型目录，默认上限 2 万条 / 12 层深度，可配置）建立路径缓存；不存在的路径（如聊天中的举例占位）保持纯文本、永不渲染成链接；扫描后新建的文件自动回退到轻量探测
- ✍️ **提示模型用相对路径:行号引用代码**：宿主通过 DSH 公开的 `systemPrompt.section()` 注入一行极简指令（可用配置 `codeRefPrompt` 关闭），让模型倾向于输出 `src/main.ts:42-56` 这类格式
- 🔌 **文件预览器新字段**：`FileViewerProps` 新增可选 `jumpLine`（`{ start, end }`），文本类预览器可据此滚动高亮行范围

**🐛 修复**

- 🖱️ **编辑器选中代码不再跳转**：修复跳转目标随每次重渲染重新应用、把用户选区重置并滚回跳转行的问题（`readJumpMeta` 记忆化 + 同目标跳转去重）
