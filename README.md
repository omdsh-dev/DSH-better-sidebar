...
- **🧩 任务管理页**：工作流图（分层节点 + 连线，缩放平移，运行中节点带「图标 + 工具 + 参数」实时行）/ 经典树双视图一键切换；workflow run 挂在发起代理下、成员按相位分框；Agent Teams（实验）的 roster 富化节点、头部 chip 打开共享任务板（完成/重开/删除/改派/新建/编辑，CAS 冲突自动刷新）；已完成叶子自动折叠为聚合节点；后台任务收进底部抽屉（代理 ≥8 自动折叠），输出走锚定浮窗（事件回放生读，不碰模型游标），两击终止。视觉基座是 **shadcn/ui + Tailwind v4**（组件源码 vendoring 在 `src/client/ui/`，shadcn 语义令牌桥接到 DSH 的 `--dsw-*`）：层级只用 1px 分隔线与表面阶梯表达、**静态面板不投影**（阴影只留给浮层），空态用 `Empty`，图标全部走宿主 primitives——深浅主题与各皮肤自动跟随
...
| **🌐 内嵌浏览器**<br/><sub>多开网页 tab：后退 / 前进 / 刷新 / 地址栏；内容运行在**不透明源沙箱 iframe**（界面实时显示沙箱状态，可按页面临时解锁）；聊天里的外链点击可被接管到侧边栏打开（按协议分流，可配）。</sub><br/><div align="center"><img width="420" alt="内嵌浏览器" src="https://github.com/user-attachments/assets/9bc6b65a-64fc-4942-a685-76e391e55606" /></div> | **🧩 任务页：工作流图 + 团队任务板 + 后台任务**<br/><sub>会话树的工作流图（分层节点 + 连线，缩放平移，运行节点实时行）与经典树双视图；workflow run 与 Agent Teams 共享任务板（CAS 操作）入图；已完成节点折叠聚合；后台任务收进底部抽屉（代理多自动折叠），输出锚定浮窗、两击终止；新子代理 / 新任务可自动激活任务页（可关），默认视图可在设置选图或树。视觉基座为 **shadcn/ui + Tailwind v4**（vendored 组件 + 令牌桥接），静态面板无投影。</sub><br/><div align="center"><img width="420" alt="任务页：子代理拓扑" src="https://github.com/user-attachments/assets/dcd8ed2f-59fa-405b-937b-2d250f5034dd" /></div> |
