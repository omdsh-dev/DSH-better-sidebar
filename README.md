...
- **⚙️ 声明式设置**：设置页「侧边卡片」逐项独立开关，二级设置经齿轮弹窗；企业部署可在挂载行写 `config.adminManaged: true`——「侧边卡片」分区不再出现在设置页、`settings.update` 路由拒绝写入（403），偏好只由宿主 `settings.yaml` 里的 `dsh-better-sidebar` 段决定
