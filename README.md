...
- **🧩 任务管理页**：工作流图（分层节点 + 连线，缩放平移，运行中节点带「图标 + 工具 + 参数」实时行）/ 经典树双视图一键切换；workflow run 挂在发起代理下、成员按相位分框；Agent Teams（实验）的 roster 富化节点、头部 chip 打开共享任务板（完成/重开/删除/改派/新建/编辑，CAS 冲突自动刷新）；已完成叶子自动折叠为聚合节点；后台任务收进底部抽屉（代理 ≥8 自动折叠），输出走锚定浮窗（事件回放生读，不碰模型游标），两击终止
