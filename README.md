...
- **🗂️ 文件工作台**：资源管理器（懒加载目录树；软链接按目标类型展示——目录软链接可展开、失效链接标红）+ CodeMirror 编辑器；图片 / Markdown / HTML / PDF / Office 内联预览；右键菜单支持新建目录/文件、重命名、删除（确认后移入回收站）、在 Finder 中查看、目录内按文件名或内容递归查找
...
| 新建 / 重命名 / 删除 | 右键行或空白处 → 新建目录/新建文件 / 重命名 / 删除（确认后移入回收站） |
| 在 Finder 中查看 | 右键行 → 在 Finder 中查看 |
| 目录内查找 | 右键行或空白处 → 查找功能（按文件名或文件内容递归查找） |
