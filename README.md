...
- **🗂️ 资源管理器**：懒加载目录树（根 = 会话 cwd），点击打开、`@文件` 引用、右键复制路径；右键目录「在资源管理器中打开」、右键文件「在资源管理器中显示 / 使用默认应用打开」（Windows / macOS / Linux 全适配）
...
| 在资源管理器中打开/显示 | 右键目录/文件 → 在资源管理器中打开 |
| 使用默认应用打开 | 右键文件 → 使用默认应用打开 |
| 文件页更多操作 | 文件查看页右上角 `⋯` → 复制路径 / 在文件管理器中显示 / 默认应用打开 |
