...
- **🌐 Embedded Browser**: multiple web tabs with back, forward, and refresh; content runs in a sandboxed iframe. Enter a local app such as `localhost:5173`, then use **Trust and open** to grant that exact address and port. External links route by protocol. HTTP opens in the sidebar, and HTTPS opens in the system browser by default.
...
- **Local web apps open from the Browser tab**: enter `localhost:5173` or a full loopback URL. The Browser asks you to trust the exact address and port, saves the choice, and loads the app with the origin permissions needed by Vite modules, fetch, and hot reload. Restored URLs use the same check. The server never probes loopback addresses.
...
- 🖥️ **Browser loopback allowlist** ([#365](https://github.com/omdsh-dev/DSH-better-sidebar/pull/365)): the side-card setting `browserAllowedLoopback` accepts comma-separated host or host:port entries. Bare hosts match every port, and host:port entries match one port. Trusted local pages get the iframe `allow-same-origin` token, which local dev servers need for modules, hot reload, and fetch. The page stays cross-origin to the GUI. The Browser now adds exact entries through **Trust and open**, while the server refuses every loopback probe.
...
- HTML previews and remote browser pages render in opaque-origin sandboxed iframes. The sandbox has no `allow-top-navigation`, sends no referrer, and grants no permissions. A trusted local web app gets `allow-same-origin` for its own address, but stays cross-origin to the DSH GUI. The address bar rejects `javascript:`, `data:`, and `file:` URLs. Local addresses require an exact **Trust and open** grant. The server never probes loopback addresses.
