...
- **⚙️ Declarative Settings**: per-item toggles in the "Side Cards" settings section, secondary settings via the gear dialog; an enterprise deployment can set `config.adminManaged: true` on the mount row — the Side card section stays out of the settings page and `settings.update` refuses writes (403), so the preferences are exactly the `dsh-better-sidebar` block of the host `settings.yaml`
