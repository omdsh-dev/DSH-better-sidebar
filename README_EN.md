...
- 🐛 **Fixed the dead "open chat files in the sidebar" takeover on alpha.2/alpha.3** ([#500](https://github.com/omdsh-dev/DSH-better-sidebar/issues/500)): the chat file-open interception used to wrap `ctx.workspaces.openPath` — an entry that **does not exist** on 0.1.2-alpha hosts (the WorkspaceController only has create/rename/delete/archive, and ui-chat's `openFile` actually calls `ctx.remote.session.openWorkspacePath`), so wrapping a method nobody calls meant the takeover never ran and `interceptOpenPath` was dead weight. The wrapper now targets `ctx.remote.session.openWorkspacePath` (the only production caller is ui-chat's apply.ts) and the client half's `inject` swapped `'workspaces'` for `'remote'`; intercepted calls resolve as `{ ok: true, value: { opened: true } }` (callers only check `result.ok`), and the "Show in folder" folder-reveal gesture still routes to the explorer.
