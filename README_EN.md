...
> 📝 **Note**: this release adds "Position compat mode" — reserves top space for the native Windows title bar (top-right), shifting the sidebar buttons and content below it (0–120px, customizable in the gear popup). The mode is **on by default** (so the sidebar's two top-right toggle buttons no longer overlap the native caption's three window buttons); non-Windows users can turn it off in settings. It ships together with the service API base introduced since v0.12.0: capability detection, state subscription, tab badges, lifecycle callbacks, external-link claiming (`urlTarget`) and plugin-owned settings are all in place for deep third-party integration; settings gained the "Add Plugins" recommended catalog. See the table below for details.

| Feature | Description | Screenshot |
|---|---|---|
| 📐 Position compat mode | "Position compatibility mode" setting (on by default): reserves top space for the native Windows title bar (top-right) so the sidebar's two top-right toggle buttons and content sit below it, instead of overlapping the native caption's three window buttons; the shift distance is customizable in the gear popup (0–120px) | |
