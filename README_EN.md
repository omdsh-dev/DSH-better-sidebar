...
- 🐛 **Opening the right panel no longer hides the official left sidebar** (fixes "the host left sidebar auto-collapses when the panel is open / when switching conversations, forcing an extra expand step"): DSH's AppFrame does not decide its narrow-viewport mode from `window.innerWidth` — it observes the **frame's own box** with a ResizeObserver and auto-collapses the host left sidebar below 1024px (`SIDEBAR_AUTO_COLLAPSE`). The old layout push squeezed `#root` itself, so opening the right panel pushed the frame under 1024 and the official left sidebar got folded away (reproducible at common laptop widths like 1280/1366). Fix: the width push moved off `#root` onto **the center column only** (`margin-right` on `[data-pane="conversation"]`, the same anchor the bottom panel uses) — the frame box keeps its width, the host breakpoint never fires, and the left sidebar stays visible; the conversation still yields exactly the panel's width (the VSCode-style squeeze is unchanged), with the 1:1 drag tracking and bottom-panel gluing guarded by the e2e regression lanes
